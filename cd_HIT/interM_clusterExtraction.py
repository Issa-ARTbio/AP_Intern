...
'''Extraire les noms des proteines dans les clusters intermediaires dans la sortie du MCL programme
Renvoit pour chaque cluster un fichier fasta correspondt aux sequences de chaque proteine dans le cluster

Usage
======
python3 interM_clusterExtraction.py
'''
...
def read_mcl_output(groups, proteomes_name):
    '''renvoit un dict avec les clusters sans duplications des Proteines
    key: cluster_id
    value = liste de clusters avec une proteine/espece (NB: pas forcement la proteine ancestrale)
    '''
    dico_cluster = {}
    with open (groups, 'r') as clusters:
        ...
        for line in clusters:
            ...
            #make a dict to count duplications
            dico_proteome_name = {}
            ...
            for name in my_cluster:
                if 1 < len(dico_proteome_name) < len(proteomes_name):
                    tmpr = name.split('|')
                    proteome = tmpr[0]
                # remove duplicates sequences; for each specie we kept one protein by creating a dict
        ...
        print('Nombre de cluster Total = ', nb_cluster)
    return dico_cluster
...
def cluster_biomineral (dico_cluster, biomineral_sp, proteomes_name):
    ...
    for dico_cluster in dico_duplication:
        ...
        if set(biomineral_sp) >= set(dico_cluster.keys()) and 1 == len(dico_cluster):
            cluster_biom.append(value)

        #Test if all biomineralizing species are in cluster
        if set(biomineral_sp) <= set(dico_cluster.keys()): #  and len(dico_cluster)>6 (enlever le cluster candidat du output)
            cluster_biomPlus.append(value)


    print(len(cluster_biomPlus))
...
if __name__ == '__main__':
    ...
    write_cluster_file (cluster_biom_plus, fas, outf)
